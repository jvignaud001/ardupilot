 ...
 *
 *
 * Routines d'initialisation et d'appel pour le mode de vol IMS2
 * Modification des PIDs
 *
 */
...
Correcteur_2nd_Ordre_Discret pid_roll2(32.2486,-64.1172,31.8696,1.8397,-0.83971);

// PID PITCH : y(n)=35.9066.x(n)-71.3902.x(n-1)+35.4847.x(n-2)+1.8397.y(n-1)-0.83971.y(n-2)
Correcteur_2nd_Ordre_Discret pid_pitch2(35.9066,-71.3902,35.4847,1.8397,-0.83971);

// PI R : y(n)=0.65646.x(n)-0.654.x(n-1)+1.y(n-1)
Correcteur_1er_Ordre_Discret pi_yaw2(0.65646,-0.654,1);

// ofstream est utilisé pour écrire un fichier CSV nommé IMS2_CSV_LOG.dat, celui-ci contiendra toutes les informations de vol
std::ofstream outf2;

// ---------------------------------------------------------------------------------------------
// ims2_init - Routine d'initialisation du mode de vol IMS2
// ---------------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------------
// ims2_init - Routine d'appel du mode de vol IMS2 à exécuter à 400Hz
// ---------------------------------------------------------------------------------------------
void Copter::ims2_run()
{
    ...
    } else // Sinon cela veut dire que les moteurs sont armés et que le drone est prêt à décoller
    {
        if (fichier_log_ouvert==false) { // Si le fichier de log n'est pas encore ouvert alors l'ouvrir
            ...
            avec pour nom : "IMS2_CSV_LOG-<année>-<mois>-<jour>-<heure>-<minute>-<seconde>.dat"*/
            strcpy(log_file_name,"/home/pi/ardupilot/build/navio2/bin/IMS2_CSV_LOG-");
            strcat(log_file_name,buffer);
            strcat(log_file_name,".dat");

            outf2.open(log_file_name); // Création d'un fichier de log unique

            // Ecriture d'une entête pour savoir à quoi correspond les données
            outf2 << "AHRS.Roll,AHRS.Pitch,AHRS.Yaw,AHRS.P,AHRS.Q,AHRS.R,RC.Roll,RC.Pitch,RC.R,RC.Thrust,Uphi,Utheta,Ur,w1,w2,w3,w4,w1_pwm,w2_pwm,w3_pwm,w4_pwm,pilot_throttle_scaled" << std::endl;
    ...
    // Calcul PID Roll
    pid_roll2.cycle(target_roll_rad-ahrs.roll+offset_ahrs_roll);
    // Calcul PID Pitch
    pid_pitch2.cycle(target_pitch_rad-ahrs.pitch+offset_ahrs_pitch);
    // Calcul PID Yaw
    pi_yaw2.cycle(target_yaw_rate_rad-ahrs.get_gyro().z+offset_ahrs_yaw);

    // Calcul PID Roll - Version sans prise en compte des offset de calibrage de l'AHRS au niveau du calcul de l'erreur
    //pid_roll2.cycle(target_roll_rad-ahrs.roll);
    // Calcul PID Pitch
    //pid_pitch2.cycle(target_pitch_rad-ahrs.pitch);
    // Calcul PID Yaw
    //pi_yaw2.cycle(target_yaw_rate_rad-ahrs.get_gyro().z);

    // Assignation des sorties de PIDs
    u_phi=pid_roll2.getyn();
    u_theta=pid_pitch2.getyn();
    u_r=pi_yaw2.getyn();
    ...
    w1_pwm=((w1*(pwm_max-pwm_min))/ROTATION_MAX)+pwm_min;
    w2_pwm=((w2*(pwm_max-pwm_min))/ROTATION_MAX)+pwm_min;
    w3_pwm=((w3*(pwm_max-pwm_min))/ROTATION_MAX)+pwm_min;
    w4_pwm=((w4*(pwm_max-pwm_min))/ROTATION_MAX)+pwm_min;
    ...
    // "AHRS.Roll,AHRS.Pitch,AHRS.Yaw,AHRS.P,AHRS.Q,AHRS.R,RC.Roll,RC.Pitch,RC.R,RC.Thrust,Uphi,Utheta,Ur,w1,w2,w3,w4,w1_pwm,w2_pwm,w3_pwm,w4_pwm,pilot_throttle_scaled"
        ...
        outf2 << ahrs.yaw;
        outf2 << ",";
        outf2 << ahrs.get_gyro().x;
        outf2 << ",";
        outf2 << ahrs.get_gyro().y;
        outf2 << ",";
        outf2 << ahrs.get_gyro().z;
        outf2 << ",";
        outf2 << target_roll_rad;
        outf2 << ",";
        outf2 << target_pitch_rad;
        outf2 << ",";
        outf2 << target_yaw_rate_rad;
        outf2 << ",";
        outf2 << target_throttle_newton;
        outf2 << ",";
        outf2 << pid_roll2.getyn();
        outf2 << ",";
        outf2 << pid_pitch2.getyn();
        outf2 << ",";
        outf2 << pi_yaw2.getyn();
        outf2 << ",";
        outf2 << w1;
